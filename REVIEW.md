...
- [x] **A1 `@field` 编译期字段名 vs 运行时 `f->name`**
      `06-reflection.md:238` 要求名字编译期已知；
      `08-iteration.md:363` 与 `10-macros.md:22` 在运行时循环里传 `f->name`。
...
- [x] **A3 切片字段可写性两种说法**
      ...
      `08-iteration.md:45-47` 通过 `*mut Self` 写 `self->ptr`/`self->len`，两种规则下都不成立。
      结论：统一到字段级 `mut`；切片的 `ptr`/`len` 天生是 `mut` 字段（推进视窗不写被借数据，
      与「`[]T` 元素不可写」不冲突）；整体赋值仍由绑定的 `mut` 决定。
      已落地：`01-types.md` 该段改写、`08-iteration.md` 的 `Enumerate` 字段补 `mut`。
      状态：已定
...
- [x] **A5 拥有式数组迭代 = 从索引位置移动**
      `08-iteration.md:240,123-127` 产出 `T`；`03-move.md:50-53` 禁止从位置移出。
      未说明元素如何离开数组（`@take`？）。
...
- [x] **B3 `EnumField` 无载荷类型** — `06-reflection.md:103-106`；无法描述 `Some(T)`；`TypeInfo` 缺 `type`、`?[]T`。
      ...
      已落地：`06-reflection.md` 的 `TypeInfo` 与 `EnumField`。
...
- [x] **B6 无结构体声明未引入** — `struct is_same<A, B>;`（`04:134`、`05:76`、`06:251`）。
      ...
      已落地：04（`{}` + 参数说明）、05、06。
- [x] **B7 第七章 open items 过期** — `07-match.md:144-149` 仍说 `return` 未定义（`08:257` 已定义）。
      结论：删掉该条目，开头指路句改为「Loops and `return` come in `08-iteration.md`」。
      已落地：`07-match.md`（开头 + open items）。
- [x] **C9 与 B7 重复** — `07-match.md:144-149` 与 `08` 的交叉引用问题，已随 B7 解决。
- [x] **B8 `if` 是否为表达式** — 第四章的 `#if` 出现在值位置；若按 A8 改为 `comptime if`，需明确 `if` 是表达式（与 `match` 对齐）。
      结论：是。「值是所取分支的值，两分支类型须一致」；无 `else` 时未取路径产出 `()`，
      故只作语句用。已落地：`08-iteration.md` 新增 `## If` 节。
...
- [x] C3 `06-reflection.md` `twice(twice(1))` —— 运行时那半改用 `io::read_u32()`，使其真为运行时
...
- [x] C8「枚举是变体的命名空间」—— 在 `07-match.md` 的 Patterns 段补写，而非改 09 的表述
